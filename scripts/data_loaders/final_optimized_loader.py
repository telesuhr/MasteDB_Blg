...
class OptimizedMultiExchangeLoader:
    """最適化版マルチ取引所データローダー"""
    ...
    def load_all_exchanges_data(self, days_back=3):
        """全取引所のデータをロード（最適化版）"""
        logger.info("=== 最適化版マルチ取引所データロード開始 ===")
    ...
    def _process_record(self, row, ticker_info, exchange):
        ...
        # 価格データ処理
        # PX_LASTを優先、なければPX_SETTLE
        px_last = self._safe_float(row.get('PX_LAST'))
        px_settle = self._safe_float(row.get('PX_SETTLE'))
            
        # SettlementPriceとLastPriceの設定
        # 当日データの場合: PX_SETTLEはNaNなので、PX_LASTを両方に使用
        # 過去データの場合: PX_SETTLEがあればそれを使用
        if px_settle is not None:
            settlement_price = px_settle
            last_price = px_last if px_last is not None else px_settle
        elif px_last is not None:
            settlement_price = px_last
            last_price = px_last
        else:
            # 価格データがない場合もNULLとして保存
    ...
    def _verify_loaded_data(self):
        ...
        with self.db_manager.get_connection() as conn:
            ...
            for _, row in quality_df.iterrows():
                ...
                logger.info(f"{exchange}: {int(row['総レコード数'])}件 ({int(row['営業日数'])}営業日)")
