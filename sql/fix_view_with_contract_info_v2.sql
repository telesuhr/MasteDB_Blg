-- V_CommodityPriceWithMaturityExビューを修正（日付変換エラー修正版）
...
SELECT 
    ...
    -- 契約月情報
    -- LME: マッピングからの契約月（date型）をvarchar変換
    -- CMX/SHFE: 最終取引日から導出した契約月（YYYY-MM形式）
    CASE 
        WHEN gf.ExchangeCode = 'LME' AND lm.ContractMonth IS NOT NULL 
            THEN CONVERT(varchar(7), lm.ContractMonth, 120)  -- YYYY-MM形式
        WHEN gf.ExchangeCode IN ('CMX', 'SHFE') AND gf.LastTradeableDate IS NOT NULL 
            THEN CONVERT(varchar(7), gf.LastTradeableDate, 120)  -- YYYY-MM形式
        ELSE NULL
...
WHERE TradeDate = '2025-06-06' AND ExchangeCode = 'LME'
...
SELECT TOP 5
    ...
    ContractMonth
FROM V_CommodityPriceWithMaturityEx
WHERE TradeDate = '2025-06-06' AND ExchangeCode = 'CMX'
ORDER BY GenericNumber;
...
SELECT TOP 5
    ...
    ContractMonth
FROM V_CommodityPriceWithMaturityEx
WHERE TradeDate = '2025-06-06' AND ExchangeCode = 'SHFE'
ORDER BY GenericNumber;