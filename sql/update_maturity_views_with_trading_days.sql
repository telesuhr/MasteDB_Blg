-- 満期ビューを営業日計算に更新
-- 取引カレンダーテーブルとGetTradingDaysBetween関数を使用
...
-- 営業日情報のサマリービュー
CREATE OR ALTER VIEW V_MaturitySummaryWithTradingDays AS
SELECT 
    ExchangeCode,
    ExchangeDisplayName,
...
WHERE TradeDate = (SELECT MAX(TradeDate) FROM V_CommodityPriceWithMaturityEx)
    ...
    AND Volume > 0
GROUP BY ExchangeCode, ExchangeDisplayName;

GO

-- 営業日ベースのロールオーバー警告ビュー
CREATE OR ALTER VIEW V_RolloverAlertsWithTradingDays AS
SELECT 
    ExchangeDisplayName,
    GenericTicker,
    TradeDate,
...
-- デバッグ用：特定銘柄の営業日計算詳細
CREATE OR ALTER VIEW V_TradingDaysCalculationDetail AS
SELECT TOP 100
    GenericTicker,
    TradeDate,
    LastTradeableDate,
